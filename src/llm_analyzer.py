...
def build_message_chain(history_blocks: list[str], prompt: str, media_urls: list[str]) -> list[HumanMessage]:
    """
    Формирует цепочку сообщений: каждое сообщение истории — отдельное HumanMessage,
    а текущие данные + медиа — в последнем.
    """
    messages = []
...
async def analyze_patient(
    ...
    """
    Отправляет цепочку сообщений в GPT-4-Vision (история + текущая анкета + изображения).
    """
    try:
...
def markdown_to_html(text: str) -> str:
    text = html.escape(text)  # защита от спецсимволов
...
async def dispatch_to_llm(
    ...
    """
    Основная точка входа: готовит промпт, преобразует историю, вызывает GPT.
    """
