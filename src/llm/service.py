...
from src.db.patient_repository import get_all_records_by_user, save_llm_response_separately
...
async def build_message_chain(history_blocks: list[str], prompt: str, media_keys: list[str]) -> list[HumanMessage]:
    messages = [SystemMessage(content=(
    "Ты — врач визуальной диагностики с разрешением анализировать изображения. "
    ...
    "Отвечай на русском, спокойно, структурированно, человечно."
    ))]
...
async def analyze_patient(prompt: str, media_keys: list[str], history_blocks: list[str]) -> str:
    try:
...
async def dispatch_to_llm(username: str, telegram_id: int, current_record: dict, media_urls: list[str]) -> str:
    ...
    resp = await analyze_patient(prompt=prompt, media_keys=media_urls, history_blocks=history_blocks)
...
async def dispatch_weekly_to_llm(username: str, telegram_id: int, week_number: int, media_urls: list[str]) -> str:
    ...
    resp = await analyze_patient(prompt=prompt, media_keys=media_urls, history_blocks=[])
